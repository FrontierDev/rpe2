...
- bombs, grenades, mines, dynamite and comparable one-use explosives;
- guns;
- armour / goggles and other wearable armour pieces;
- deflector-style consumables, represented in the same item-use family as Flame Deflector where the effect can be represented;
- ammunition.
...
Do not implement components or generic devices merely for source completeness. In particular, this slice excludes Bronze Framework, Gyromatic Micro-Adjustor, Solid Blasting Powder, Mithril Tube, Unstable Trigger, Mithril Casing, Portable Bronze Mortar, Goblin Mortar, Gnomish Cloaking Device, Mechanical Dragonling, Gnomish Shrink Ray, Gnomish Net-o-Matic Projector, Large Seaforium Charge, target dummies, pets, scopes, repair/utility gadgets, and other non-wearable devices.

`Explosive Sheep` is also excluded because its recipe requires exactly 150 Engineering, which belongs to the previous boundary rather than this slice.

`The Big One` is excluded because the vanilla recipe requires 235 Engineering even though some later/current item data displays a 225 use requirement.
...
- Manually activated item effects use `Item.useSpellRef -> Spell` when the generic Spell/Aura system can represent them.
...
## Current canonical material refs
...
- Gnomish Goggles consumes the new Fire Goggles output from this slice.
...
Current `client/client_Crafting.lua` still calculates:
...
| 195 | Goblin Land Mine | 4395 | Schematic 4416, world drop | Heavy Blasting Powder x3; Iron Bar x2; Gyrochronatom x1 | Heavy Stone x3; Iron Bar x2; Gold Bar x1; Blacksmith Hammer tool | 1 | 66,375 |
...
### Deflector / explosives

- **Ice Deflector** — item level 31, required level 21, 5 charges in Classic. RPE should follow the Flame Deflector consumable pattern rather than model literal charges. Its source effect absorbs 600 Frost damage for 1 minute on a 15-minute cooldown. The current generic Aura schema has no damage-absorb/shield effect, so its exact active effect is blocked; do not substitute Frost Resistance because that changes semantics.
- **Solid Dynamite** — item level 35, stack 20, Engineering 175; midpoint source damage `250 Fire`; RPE multi-target 1–5, shared 5-turn `engineering_explosive` cooldown.
- **Iron Grenade** — item level 35, stack 10, Engineering 175; midpoint `175 Fire`; one-turn break-on-damage control Aura; multi-target 1–5; shared explosive cooldown.
- **Flash Bomb** — item level 37, required level 27, stack 5; source causes Beasts in the radius to flee for 10 seconds. RPE currently has no creature-family predicate / fear-movement behavior that can reproduce this without affecting non-Beasts, so active behavior is blocked rather than generalized inaccurately.
- **Big Iron Bomb** — item level 43, stack 10, Engineering 190; midpoint `175 Fire`; one-turn break-on-damage control Aura; multi-target 1–5; shared explosive cooldown.
- **Goblin Land Mine** — item level 39, stack 10, Engineering 195; source places a one-minute proximity mine dealing midpoint `450 Fire` on trigger. The current item-use/Spell model has no persistent placed proximity trigger, so active behavior is blocked rather than converted into an immediate bomb.
- **EZ-Thro Dynamite II** — required level 30 in vanilla-era data, stack 20; non-engineer dynamite analogue. Represent as a normal consumable explosive with no Engineering use condition, midpoint/source fixed damage prepared as `120 Fire`, multi-target 1–5, shared explosive cooldown.
- **Goblin Sapper Charge** — item level 41, stack 10, Engineering 205. Source deals midpoint `600 Fire` to nearby enemies and midpoint `500` damage to the user; represent with an enemy multi-target Fire component plus a caster-target self-damage component, shared 5-turn explosive cooldown.
...
- **Gnomish Rocket Boots** — ilvl 45, BoE Cloth Feet, 41 Armor, Engineering 225. Source use provides a large movement-speed boost with malfunction risk. Movement Speed itself exists, but Classic gives no stable numeric magnitude in the item data and the malfunction behavior is not generically modeled; do not guess a magnitude.
- **Goblin Rocket Boots** — ilvl 45, BoE Cloth Feet, 41 Armor, Engineering 225. Same preparation decision as Gnomish Rocket Boots: wearable item is implementable, exact active effect is blocked pending a defined movement-speed magnitude/failure model.
...
- **Spellpower Goggles Xtreme** — ilvl 43, BoE Cloth Head, 46 Armor, +21 Spell Power, source item requires Engineering 215 while the recipe itself is skill 225. RPE recipe threshold remains 225; item condition should preserve the source equip/use Engineering requirement prepared for the Item issue.
...
4. EZ-Thro Dynamite II — 120 Fire, multi 1–5, no Engineering use requirement.
...
Do **not** implement inaccurate substitute effects for Ice Deflector, Flash Bomb, Goblin Land Mine, Goblin Construction Helmet, Gnomish Harm Prevention Belt, Catseye Ultra Goggles, the two Rocket Boots, or Parachute Cloak. Their limitations are recorded below.
...
- Current `Aura.lua` supports damage/heal/stat/skill/control/apply-aura/resource effects but no absorb/shield primitive and no placed-world-object/proximity trigger.
...
Primary source verification used vanilla Classic Wowhead Engineering recipe tables plus the individual Classic Item/Spell records for the focused outputs. The range boundary was checked against the recipe skill, not later/current item-use requirements. This is why `The Big One` is excluded at recipe skill 235 despite item data that may show Engineering 225.

## Incomplete / Blocked Items and Recipes

The **Items and Recipes themselves are fully identified** for this focused scope. The following gameplay effects cannot currently be represented exactly and must remain explicit implementation blockers rather than receiving inaccurate substitutes:
...
| Goblin Land Mine | item/recipe/source effect | persistent placed proximity trigger | no placed-world-object/proximity Spell primitive | generic delayed/proximity trigger system |
...
| Gnomish Rocket Boots | wearable stats/recipe | exact speed boost + malfunction | no authoritative numeric magnitude; failure semantics absent | define RPE movement-speed mapping and generic malfunction behavior |
| Goblin Rocket Boots | wearable stats/recipe | exact speed boost + explosion malfunction | same | same |
