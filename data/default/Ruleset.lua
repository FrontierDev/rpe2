...
local function syncDefaultRuleset()
    ...
    -- The packaged Core ruleset is the fallback for any character without an
    -- explicit active ruleset. Never replace another ruleset the player chose.
    if Database.GetActiveRulesetId() == nil then
        Database.SetActiveRulesetId(DefaultRuleset.id)
    end
