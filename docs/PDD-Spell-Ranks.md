...
| Seal of Righteousness proc | Seal costs 5% Base Mana; guaranteed on basic attacks | `28.1667 + 0.845 × Spell Power` per proc | **Aligned to the current damage scale.** Uses the same per-proc magnitude as one Ignite tick, but as guaranteed basic-attack bonus damage. |
...
| Molten Armor retaliation | 5% Base Mana; 10-turn self Aura; triggers when hit by melee; also grants fixed +5 Spell Crit. Chance | `28.1667 + 0.845 × Spell Power` per trigger | **Rebalanced to the current damage scale.** Uses the same per-trigger damage as one Ignite tick. The damage is rank-scaled; the +5 Spell Crit. Chance is explicitly fixed. |
...
| Holy Shield proc | 5% Base Mana; 3-turn Aura; +30 Block Chance; proc on successful block | `96 + 0.40 × Spell Power` per successful block | **Rebalanced with the current damage scale.** The balance risk is repeated proc frequency, not the one-shot coefficient. Fixed +30 Block Chance must remain fixed when the proc damage ranks. |
...
| Pyroblast | 31.8% Base Mana; direct hit plus 5-turn DoT | Direct: `258.188 + 2.6776 × Spell Power`; DoT: `17.68 + 0.884 × Spell Power` per turn | **Keep the direct hit.** The DoT coefficient is applied on every tick, so the full five-turn spell has much larger total Spell Power scaling than the direct component alone suggests. |
| Arcane Barrage | 4 Arcane Charges | `191.25 + 1.9126 × Spell Power` | **Keep.** It is a true special-resource spender and also enables Arcane Missiles. |
| Arcane Surge | 100% Max Mana; 10-turn cooldown; restores 8% Max Mana per turn for 10 turns | `353.813 + 3.5382 × Spell Power` | **Keep.** The up-front cost is extreme, but up to 80% Max Mana is returned over the Aura duration. |
...
| Shadow Word: Death | 13.6% Base Mana; target must be at or below 20% health | `225 + 2.25 × Spell Power` | **Keep.** The execute gate justifies the higher damage relative to ordinary Priest nukes. |
