...
| Seal of Righteousness proc | Seal costs 5% Base Mana; guaranteed on basic attacks | `8 + 0.20 × Spell Power` per proc | **Finalized baseline.** Reliable Spell-Power-oriented seal with steady supplemental Holy damage. |
| Seal of Fury proc | Seal costs 5% Base Mana; guaranteed on basic attacks; also applies Righteous Indignation on melee hits | `0 + 0.10 × Melee Attack Power` per proc | **Finalized baseline.** Reliable but lower sustained damage because Fury also carries defensive utility. The separate Righteous Indignation absorb must not be increased accidentally just because the proc damage ranks. |
| Seal of Command proc | Seal costs 5% Base Mana; 70% chance on basic-attack hit | `0 + 0.30 × Melee Attack Power` per successful proc | **Finalized baseline.** Deliberately burstier than Righteousness/Fury: lower proc reliability, materially higher per-proc damage. At 70% proc chance its expected AP contribution is `0.21 × AP` per basic hit before rank scaling. |
