...
| Seal of Righteousness proc | Seal costs 5% Base Mana; guaranteed on basic attacks | `6 + 0.15 × Spell Power` per proc | **Rebalanced.** Reliable Spell-Power-oriented seal. The previous `6 + 1.00 × Spell Power` per-hit scaling was an outlier for a guaranteed recurring proc. |
| Seal of Fury proc | Seal costs 5% Base Mana; guaranteed on basic attacks; also applies Righteous Indignation on melee hits | `0 + 0.08 × Melee Attack Power` per proc | **Rebalanced.** Lower sustained damage than the offensive seals because Fury also carries defensive utility. The separate Righteous Indignation absorb must not be increased accidentally just because the proc damage ranks. |
| Seal of Command proc | Seal costs 5% Base Mana; 70% chance on basic-attack hit | `0 + 0.20 × Melee Attack Power` per successful proc | **Rebalanced.** Deliberately burstier than Righteousness/Fury: lower proc reliability, materially higher per-proc damage. At 70% proc chance its expected AP contribution is `0.14 × AP` per basic hit before rank scaling. |
